	...
	private void initialize() throws SQLException {
		...
		table.addMouseListener(new java.awt.event.MouseAdapter() {
		    ...
		    public void mouseClicked(java.awt.event.MouseEvent evt) {
		        ...
		        if (row >= 0 && col >= 0) {
		        	String TablaPaiCod = table.getModel().getValueAt(row, 0).toString();
		        	String TablaPaiDes = table.getModel().getValueAt(row, 1).toString();
		        	String TablaPaiEstReg = table.getModel().getValueAt(row, 2).toString();
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String Paicodigo = table.getModel().getValueAt(row, column).toString();
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzz_pais SET PaiCod ='"+PaiCod+"',PaiDes ='"+PaiDes+"',PaiEstReg ='"+PaiEstReg+"' WHERE PaiCod = "+Paicodigo+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {
				...
				switch (result) {
		         case JOptionPane.YES_OPTION:
		        	int column = 0;
					int row = table.getSelectedRow();
					String PaiCod = table.getModel().getValueAt(row, column).toString();
